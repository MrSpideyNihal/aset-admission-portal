  ...
  return (
    <div style={{ maxWidth: '1280px', margin: '0 auto', padding: '40px 24px' }}>
      ...
      <section style={{ textAlign: 'center', padding: '80px 0 60px 0', position: 'relative' }}>
        ...
        <p style={{ fontSize: '1.4rem', color: 'var(--text-secondary)', marginBottom: '32px', fontWeight: 500 }}>
      ...
      <section style={{ marginBottom: '80px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '24px' }}>
          <GlassCard hoverable style={{ padding: '32px', textAlign: 'center' }}>
            ...
            <h3 style={{ fontSize: '2.5rem', fontWeight: 800, marginBottom: '8px', color: 'var(--text-primary)' }}>{stats.students}+</h3>
            <p style={{ color: 'var(--text-secondary)', fontSize: '0.95rem' }}>Active Students</p>
          </GlassCard>
          <GlassCard hoverable style={{ padding: '32px', textAlign: 'center' }}>
            <Award size={40} style={{ color: '#6C63FF', margin: '0 auto 16px auto' }} />
            <h3 style={{ fontSize: '2.5rem', fontWeight: 800, marginBottom: '8px', color: 'var(--text-primary)' }}>{stats.years}+</h3>
            <p style={{ color: 'var(--text-secondary)', fontSize: '0.95rem' }}>Years of Excellence</p>
          </GlassCard>
          <GlassCard hoverable style={{ padding: '32px', textAlign: 'center' }}>
            <Trophy size={40} style={{ color: '#FFD93D', margin: '0 auto 16px auto' }} />
            <h3 style={{ fontSize: '2.5rem', fontWeight: 800, marginBottom: '8px', color: 'var(--text-primary)' }}>{stats.placements}%</h3>
            <p style={{ color: 'var(--text-secondary)', fontSize: '0.95rem' }}>Placement Rate</p>
          </GlassCard>
          <GlassCard hoverable style={{ padding: '32px', textAlign: 'center' }}>
            <Settings size={40} style={{ color: '#2ECC71', margin: '0 auto 16px auto' }} />
            <h3 style={{ fontSize: '2.5rem', fontWeight: 800, marginBottom: '8px', color: 'var(--text-primary)' }}>{stats.programs}</h3>
            <p style={{ color: 'var(--text-secondary)', fontSize: '0.95rem' }}>B.Tech Programs</p>
          </GlassCard>
        </div>
      </section>
          ...
          {courses.map((course, idx) => (
            <GlassCard key={idx} hoverable style={{ padding: '32px', display: 'flex', flexDirection: 'column', height: '100%' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px' }}>
                <div style={{ padding: '10px', borderRadius: '12px', background: 'var(--glass-bg)', border: '1px solid var(--glass-border)' }}>
                  {course.icon}
                </div>
                <span className="glass-badge" style={{ background: 'var(--glass-bg)' }}>4 Year Program</span>
              </div>
              <h3 style={{ fontSize: '1.25rem', fontWeight: 700, marginBottom: '12px', lineHeight: '1.4', color: 'var(--text-primary)' }}>{course.name}</h3>
              <p style={{ color: 'var(--text-secondary)', fontSize: '0.95rem', marginBottom: '24px', flexGrow: 1 }}>{course.desc}</p>
      ...
      <section style={{ marginBottom: '80px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '24px' }}>
          <GlassCard style={{ padding: '24px' }}>
            <h3 style={{ fontSize: '1.2rem', fontWeight: 700, marginBottom: '8px', color: 'var(--accent-cyan)' }}>Modern Labs</h3>
            <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem' }}>Fully equipped advanced engineering laboratories for hands-on, practical technical learning.</p>
          </GlassCard>
          <GlassCard style={{ padding: '24px' }}>
            <h3 style={{ fontSize: '1.2rem', fontWeight: 700, marginBottom: '8px', color: 'var(--accent-purple)' }}>Experienced Faculty</h3>
            <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem' }}>Highly credentialed professors dedicated to mentoring research and engineering foundations.</p>
          </GlassCard>
          <GlassCard style={{ padding: '24px' }}>
            <h3 style={{ fontSize: '1.2rem', fontWeight: 700, marginBottom: '8px', color: 'var(--accent-green)' }}>Industry Connections</h3>
            <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem' }}>Strategic tie-ups and MoUs with tech institutions ensuring placements and internships.</p>
          </GlassCard>
          <GlassCard style={{ padding: '24px' }}>
            <h3 style={{ fontSize: '1.2rem', fontWeight: 700, marginBottom: '8px', color: 'var(--accent-yellow)' }}>Green Campus</h3>
            <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem' }}>ISO certified environment-friendly eco-campus incorporating water recycling and solar grids.</p>
          </GlassCard>
        </div>
      </section>
        ...
        <div style={{ display: 'flex', gap: '24px', overflowX: 'auto', paddingBottom: '16px', scrollSnapType: 'x mandatory' }}>
          {newsItems.map((news, idx) => (
            <GlassCard key={idx} style={{ padding: '24px', minWidth: '300px', flex: '0 0 300px', scrollSnapAlign: 'start' }}>
              <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)', fontWeight: 600 }}>{news.date}</span>
              <h3 style={{ fontSize: '1.1rem', fontWeight: 700, margin: '8px 0 12px 0', color: 'var(--accent-cyan)' }}>{news.title}</h3>
              <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem' }}>{news.desc}</p>
            </GlassCard>
          ))}
        </div>
      </section>
      ...
      {/* Footer */}
      <footer style={{ marginTop: '120px', borderTop: '1px solid var(--glass-border)', paddingTop: '48px', paddingBottom: '24px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '40px', marginBottom: '40px' }}>
          ...
          <div>
            <h4 style={{ fontWeight: 600, marginBottom: '16px', color: 'var(--text-primary)' }}>Quick Links</h4>
            <ul style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '10px' }}>
              <li><Link to="/about" style={{ color: 'var(--text-secondary)', textDecoration: 'none', fontSize: '0.9rem' }}>About Us</Link></li>
              <li><Link to="/courses" style={{ color: 'var(--text-secondary)', textDecoration: 'none', fontSize: '0.9rem' }}>B.Tech Courses</Link></li>
              <li><Link to="/contact" style={{ color: 'var(--text-secondary)', textDecoration: 'none', fontSize: '0.9rem' }}>Contact Desk</Link></li>
            </ul>
          </div>
          <div>
            <h4 style={{ fontWeight: 600, marginBottom: '16px', color: 'var(--text-primary)' }}>Contact Info</h4>
        ...
        <div style={{ textAlign: 'center', color: 'var(--text-muted)', fontSize: '0.8rem', borderTop: '1px solid var(--glass-border)', paddingTop: '24px' }}>
